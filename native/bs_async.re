...
let make_error = (loc, msg) =>
  Location.error(~loc, msg) |> extension_of_error |> Exp.extension;
...
let promise_resolve =
  Longident.parse("Js.Promise.resolve") |> Location.mknoloc;
...
let promise_recover =
  Longident.parse("JsPromise.recover") |> Location.mknoloc;
let promise_catch = Longident.parse("JsPromise.catch") |> Location.mknoloc;
let promise_map_recover =
  Longident.parse("JsPromise.map_recover") |> Location.mknoloc;
let promise_bind_catch =
  Longident.parse("JsPromise.then_catch") |> Location.mknoloc;
...
let expanded_let_multi = (mapper, mode, loc, attrs, bindings, expr) => {
  let patterns =
    List.map(({pvb_pat, _}) => mapper.pat(mapper, pvb_pat), bindings)
    |> Pat.array
  and values =
    List.map(({pvb_expr, _}) => mapper.expr(mapper, pvb_expr), bindings)
    |> Exp.array;
  ...
  Exp.apply(
    ...
    [
      (
        Asttypes.Nolabel,
        Exp.fun_(Asttypes.Nolabel, None, patterns, mapper.expr(mapper, expr))
        |> warning("-8"),
      ),
      (
        Asttypes.Nolabel,
        Exp.apply(Exp.ident(promise_all), [(Asttypes.Nolabel, values)]),
      ),
    ],
  );
};
        ...
        Exp.apply(
          ...
          [
            ...
            (
              Asttypes.Nolabel,
              Exp.apply(
                Exp.ident(promise_resolve),
                [(Asttypes.Nolabel, Exp.construct(unit, None))],
              ),
            ),
          ],
        ),
...
let expanded_match = (mapper, mode, loc, attrs, cases, expr) => {
  ...
  switch (List.filter(is_exception, cases)) {
  | [] =>
    ...
    Exp.apply(
      ...
      [
        (Asttypes.Nolabel, Exp.function_(~loc, mapper.cases(mapper, cases))),
        (Asttypes.Nolabel, mapper.expr(mapper, expr)),
      ],
    )
  | exceptions =>
    let not_exceptions = List.filter(x => (!) @@ is_exception(x), cases);
    Exp.apply(
      ~loc,
      ~attrs,
      Exp.ident(
        switch (mode) {
        | Functor => promise_map_recover
        | Monad => promise_bind_catch
        },
      ),
      [
        (
        ...
        (
          Asttypes.Nolabel,
          Exp.function_ @@
          (List.map(unwrap, exceptions) |> mapper.cases(mapper))
          @ catch_all(mode)
          |> warning("-11"),
        ),
        (Asttypes.Nolabel, mapper.expr(mapper, expr)),
...
let expanded_expression =
    (mapper, mode, {pexp_desc, pexp_loc, pexp_attributes}) =>
  switch (pexp_desc) {
  | [@implicit_arity] Pexp_let(Asttypes.Nonrecursive, [], _) =>
    /* Unlikely */
    make_error(pexp_loc, "No bindings defined in let expression")
  | [@implicit_arity] Pexp_let(Asttypes.Nonrecursive, [binding], expr) =>
    expanded_let_single(
      mapper,
      mode,
      pexp_loc,
  ...
  | [@implicit_arity] Pexp_let(Asttypes.Nonrecursive, bindings, expr) =>
    expanded_let_multi(
      mapper,
      mode,
      pexp_loc,
  ...
  | [@implicit_arity] Pexp_let(Asttypes.Recursive, _, _) =>
    make_error(pexp_loc, "Async cannot transform recursive bindings")
  | [@implicit_arity] Pexp_try(_, []) =>
    /* Unlikely */
  ...
  | _ => make_error(pexp_loc, "Async does not work here")
  };
 ...
 */
let async_transform = mapper =>
  fun
  | {
      pexp_desc:
        [@implicit_arity]
        Pexp_extension({txt: ("async" | "async'") as mode, loc}, payload),
      _,
    } =>
    switch (payload) {
    | PStr([]) =>
    ...
    | PStr([{pstr_desc: [@implicit_arity] Pstr_eval(exp, []), _}]) =>
      expanded_expression(mapper, mode_of_string(mode), exp)
    | PStr([{pstr_desc: [@implicit_arity] Pstr_eval(_, _), _}]) =>
      make_error(loc, "Async does not allow attributes")
    | PStr([_]) => make_error(loc, "Async only works with expressions")
    | PStr(_) =>
      /* Unlikely? */
      make_error(loc, "Async does not accept multiple structures")
    | _ => make_error(loc, "Async is not supported like this")
    }
  | other => default_mapper.expr(mapper, other);

/* Perhaps Ast_mapper.run_main makes a difference in run time speed? */
let () = register("async", _ => {...default_mapper, expr: async_transform});
